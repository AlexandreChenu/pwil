...
class DubinsMazeEnv(Maze, gym.Env):
    ...
    def setup(self,args):
        ...
        self.state =  np.array([0.5, 0.5, 1.57, 0.])
        self.steps = []
        self.obs_dim = 2
    ...
    def update_state(self, state, a, DT):
        ...
        if throttle > MAX_ACCELERATION:
            throttle = MAX_ACCELERATION
        elif throttle < MIN_ACCELERATION:
            throttle = MIN_ACCELERATION


        state[0] = state[0] + state[3] * math.cos(state[2]) * DT
        state[1] = state[1] + state[3] * math.sin(state[2]) * DT
        new_orientation = state[2] + state[3] / WB * math.tan(steer) * DT
        speed = state[3] + throttle * DT
